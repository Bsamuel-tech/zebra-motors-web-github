...
// Rwanda's 2006 administrative reform renamed several well-known towns
// (real, documented history, not a guess), but both the old and new names
// are still in everyday use, a customer or a Zebra staff member typing
// "Gisenyi" means the same place OpenStreetMap mostly indexes as "Rubavu".
// This is a real, well-known naming fact worth handling, unlike a business
// fact that would need Zebra to confirm, so it is safe to encode directly.
// If a search for the name as typed finds nothing, geocodePlace() retries
// once with the paired name before giving up, it never changes what was
// actually searched for in the result, only what gets sent to Nominatim.
const RWANDA_PLACE_RENAMES = {
  gisenyi: "Rubavu",
  rubavu: "Gisenyi",
  kibuye: "Karongi",
  karongi: "Kibuye",
...
function renamedAlternative(query) {
  const normalized = query.trim().toLowerCase();
  for (const [oldName, newName] of Object.entries(RWANDA_PLACE_RENAMES)) {
    if (normalized === oldName || normalized.startsWith(`${oldName} `) || normalized.startsWith(`${oldName},`)) {
      return query.replace(new RegExp(oldName, "i"), newName);
    }
  }
  return null;
}

// One real Nominatim lookup, biased to Rwanda (countrycodes=rw) so a short,
// ambiguous local place name matches somewhere in Rwanda first rather than
// a same-named place anywhere else in the world, exactly the kind of
// mismatch a European address search never has to worry about. Returns the
// same shape geocodePlace() does, or null, never invented.
async function nominatimLookup(query) {
  await respectNominatimRateLimit();
  const url = `https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=rw&q=${encodeURIComponent(query)}`;
...
export async function geocodePlace(query) {
  ...
  try {
    let result = await nominatimLookup(query);
    if (!result) {
      const alternative = renamedAlternative(query);
      if (alternative) result = await nominatimLookup(alternative);
    }
