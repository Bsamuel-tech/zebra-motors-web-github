...
// content on a serverless host like Netlify, where nothing ever gets a
// chance to run `npm run db:seed` by hand, see the comment in client.js.
//
...
export function seedDatabase(db, env = process.env) {
  ...
  const existingSettings = db.prepare("SELECT id FROM business_settings WHERE id = 'singleton'").get();
  if (!existingSettings) {
    db.prepare(
  ...
  for (const v of VEHICLES) {
    const existing = db.prepare("SELECT id FROM vehicles WHERE slug = ?").get(v.slug);
  ...
  if (reviewCount === 0) {
    ...
    for (const r of REVIEWS) {
      insertReview.run(newId(), r.name, r.stars, r.text, "zebramotors.rw", 0, 1, nowIso());
    }
  }
  ...
  if (faqCount === 0) {
    ...
    FAQS.forEach((f, i) => {
      const now = nowIso();
      insertFaq.run(newId(), f.question, f.answer, i, now, now);
    });
  }
  ...
  if (pkgCount === 0) {
    ...
    for (const p of PACKAGES) {
      const now = nowIso();
      insertPkg.run(newId(), p.slug, p.name, p.duration, p.summary, JSON.stringify(p.includes), JSON.stringify(p.bestFor), now, now);
    }
  }
  ...
  const articleCount = db.prepare("SELECT COUNT(*) as c FROM guide_articles").get().c;
  if (articleCount === 0) {
    ...
    for (const a of ARTICLES) {
      const now = nowIso();
      insertArticle.run(newId(), a.slug, a.title, a.excerpt, JSON.stringify(a.body), now, now, now);
    }
  }
  ...
  const extrasCount = db.prepare("SELECT COUNT(*) as c FROM rental_extras").get().c;
  if (extrasCount === 0) {
    ...
    for (const e of EXTRAS) {
      const now = nowIso();
      insertExtra.run(newId(), e.key, e.name, e.description, e.pricingType, now, now);
    }
  }
  ...
  // No coordinates yet, real lat/lng needs a geocoding provider (P2), and
  // no photos, those need a real admin upload, same as vehicles.
  const DESTINATIONS = [
    { slug: "kigali", name: "Kigali", region: "Kigali", category: "CITY", description: "City orientation, memorial, markets. Roads paved and easy for self-drive." },
    { slug: "akagera-national-park", name: "Akagera National Park", region: "Eastern Province", category: "NATIONAL_PARK", description: "About 2.5 to 3 hours from Kigali. Game drives, some unpaved park roads, 4WD recommended.", recommendedVehicleCategory: "SUV" },
    { slug: "lake-kivu", name: "Lake Kivu", region: "Western Province", category: "LAKE", description: "Scenic lakeside roads via Musanze or Karongi, relaxed pace." },
    { slug: "volcanoes-national-park", name: "Volcanoes National Park", region: "Northern Province", category: "NATIONAL_PARK", description: "About 2 to 3 hours from Kigali. Early starts for gorilla trekking, permits booked separately.", recommendedVehicleCategory: "SUV" },
    { slug: "nyungwe-forest", name: "Nyungwe Forest", region: "Southern Province", category: "NATIONAL_PARK", description: "About 5 to 6 hours from Kigali via Huye. Canopy walk and forest trekking." },
  ];
  const destinationCount = db.prepare("SELECT COUNT(*) as c FROM destinations").get().c;
  if (destinationCount === 0) {
    const insertDestination = db.prepare(
      `INSERT INTO destinations (id, slug, name, description, region, category, recommended_vehicle_category, published, created_at, updated_at)
       VALUES (?,?,?,?,?,?,?,1,?,?)`
    );
    for (const d of DESTINATIONS) {
      const now = nowIso();
      insertDestination.run(newId(), d.slug, d.name, d.description, d.region, d.category, d.recommendedVehicleCategory || null, now, now);
    }
  }

  // --- Admin user ------------------------------------------------------
  // Requires ADMIN_EMAIL / ADMIN_PASSWORD to be set wherever this runs. In
  // local dev, Next.js loads .env automatically. On Netlify, these must be
  // added as real environment variables in Site settings, they are not
  // read from any committed file. Skipped (not invented) if missing.
  const adminEmail = (env.ADMIN_EMAIL || "admin@zebramotor.rw").toLowerCase();
