...
export function seedDatabase(db, env = process.env) {
  ...
  // local dev, Next.js loads .env automatically. On a host like Netlify or
  // Vercel, these must be added as real environment variables in that
  // host's own project settings, they are not read from any committed
  // file (.env is gitignored on purpose). Skipped (not invented) if
  // missing, this file never invents a password.
