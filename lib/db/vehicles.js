...
function rowToVehicle(row, photos) {
  return {
...
export async function getVehicles({ includeAll = false } = {}) {
  const db = await getDb();
  const rows = includeAll
    ? await db.prepare("SELECT * FROM vehicles ORDER BY created_at ASC").all()
    : await db
        .prepare(
          `SELECT * FROM vehicles WHERE status IN (${PUBLIC_STATUSES.map(() => "?").join(",")}) ORDER BY created_at ASC`
        )
        .all(...PUBLIC_STATUSES);
  return Promise.all(rows.map(async (row) => rowToVehicle(row, await getPhotosForVehicle(row.id))));
}

export async function getVehicleBySlug(slug) {
  const db = await getDb();
  const row = await db.prepare("SELECT * FROM vehicles WHERE slug = ?").get(slug);
  if (!row) return null;
  return rowToVehicle(row, await getPhotosForVehicle(row.id));
}

export async function getVehicleByDbId(id) {
  const db = await getDb();
...
export async function updateVehicle(dbId, patch) {
  const db = await getDb();
  const sets = [];
  const values = [];
  ...
  values.push(nowIso());
  values.push(dbId);
  await db.prepare(`UPDATE vehicles SET ${sets.join(", ")} WHERE id = ?`).run(...values);
...
export async function archiveVehicle(dbId) {
