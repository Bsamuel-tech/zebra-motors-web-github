...
export function getCachedGeocode(query) {
  const row = getDb().prepare("SELECT * FROM geocode_cache WHERE query = ?").get(query.toLowerCase().trim());
  if (!row) return null;
  return { lat: row.lat, lng: row.lng, displayName: row.display_name, provider: row.provider };
}

export function saveGeocode(query, { lat, lng, displayName, provider }) {
  const db = getDb();
  db.prepare(
...
export function getCachedRoute(cacheKey) {
  const row = getDb().prepare("SELECT * FROM route_cache WHERE cache_key = ?").get(cacheKey);
  if (!row) return null;
  return {
...
export function saveRoute(cacheKey, { distanceKm, durationMinutes, geometry, legs, provider }) {
  const db = getDb();
  db.prepare(
