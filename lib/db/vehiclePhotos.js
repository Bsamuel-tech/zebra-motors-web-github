...
export function getPhotosForVehicle(vehicleId) {
  const rows = getDb()
...
export function getPhotoById(id) {
  const row = getDb().prepare("SELECT * FROM vehicle_photos WHERE id = ?").get(id);
  return row ? rowToPhoto(row) : null;
}

export function addPhoto({ vehicleId, url, altText = "", isPrimary = false }) {
  const db = getDb();
  const id = newId();
  const now = nowIso();
  // A brand new photo becomes primary automatically if this vehicle has none yet.
  const existingCount = db.prepare("SELECT COUNT(*) as c FROM vehicle_photos WHERE vehicle_id = ?").get(vehicleId).c;
...
export function setPrimaryPhoto(id, vehicleId) {
  const db = getDb();
  db.prepare("UPDATE vehicle_photos SET is_primary = 0 WHERE vehicle_id = ?").run(vehicleId);
  db.prepare("UPDATE vehicle_photos SET is_primary = 1 WHERE id = ? AND vehicle_id = ?").run(id, vehicleId);
  return getPhotoById(id);
}
...
export function deletePhoto(id) {
  ...
  if (photo.isPrimary) {
    const next = getDb()
      ...
      .get(photo.vehicleId);
    if (next) {
      getDb().prepare("UPDATE vehicle_photos SET is_primary = 1 WHERE id = ?").run(next.id);
    }
  }
  return photo;
}
