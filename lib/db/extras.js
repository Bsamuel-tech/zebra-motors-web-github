...
export function getExtras({ activeOnly = false } = {}) {
  const rows = getDb().prepare("SELECT * FROM rental_extras ORDER BY created_at ASC").all();
...
export function getExtraById(id) {
  const row = getDb().prepare("SELECT * FROM rental_extras WHERE id = ?").get(id);
  return row ? rowToExtra(row) : null;
}

export function getExtraByKey(key) {
  const row = getDb().prepare("SELECT * FROM rental_extras WHERE key = ?").get(key);
  return row ? rowToExtra(row) : null;
}

export function updateExtra(id, { name, description, pricingType, priceRWF, active }) {
  const db = getDb();
  const existing = getExtraById(id);
  ...
  if (sets.length) {
    ...
    values.push(nowIso());
    values.push(id);
    db.prepare(`UPDATE rental_extras SET ${sets.join(", ")} WHERE id = ?`).run(...values);
  }
...
export function createExtra({ key, name, description, pricingType, priceRWF, active }) {
