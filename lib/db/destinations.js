...
async function rowToDestination(row) {
  return {
    id: row.slug,
    dbId: row.id,
    name: row.name,
    ...
    photos: await getPhotosForDestination(row.id),
...
export async function getDestinations({ publishedOnly = false, query = "" } = {}) {
  ...
  return Promise.all(rows.map(rowToDestination));
}
...
export async function recordDestinationSelection(destinationId) {
  const db = await getDb();
  const exists = await db.prepare("SELECT id FROM destinations WHERE id = ?").get(destinationId);
  if (!exists) return;
  await db
    .prepare("INSERT INTO destination_selections (id, destination_id, created_at) VALUES (?,?,?)")
    .run(newId(), destinationId, nowIso());
}

export async function getDestinationBySlug(slug) {
  const db = await getDb();
  const row = await db.prepare(`${SELECT_WITH_COUNT} WHERE d.slug = ?`).get(slug);
  return row ? rowToDestination(row) : null;
}

export async function getDestinationByDbId(id) {
  const db = await getDb();
  const row = await db.prepare(`${SELECT_WITH_COUNT} WHERE d.id = ?`).get(id);
  return row ? rowToDestination(row) : null;
}
...
export async function updateDestination(dbId, patch) {
  const db = await getDb();
  const sets = [];
  const values = [];
  ...
  values.push(nowIso());
  values.push(dbId);
  await db.prepare(`UPDATE destinations SET ${sets.join(", ")} WHERE id = ?`).run(...values);
...
export async function unpublishDestination(dbId) {
...
export async function logCustomDestinationRequest({ name, notes = "", source = "trip_planner" }) {
...
export async function getCustomDestinationRequests() {
  const db = await getDb();
  return db.prepare("SELECT * FROM custom_destination_requests ORDER BY created_at DESC").all();
}
