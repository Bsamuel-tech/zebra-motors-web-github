...
export async function GET() {
  ...
  return NextResponse.json({ packages: getPackages({ publishedOnly: !session }) });
}
...
export async function POST(request) {
  ...
  const pkg = createPackage({ ...input, slug });
  logAction({ userId: session.userId, action: "CREATE", entityType: "Package", entityId: pkg.id, detail: pkg.name });
