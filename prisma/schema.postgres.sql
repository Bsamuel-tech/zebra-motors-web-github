-- ---------------------------------------------------------------------------
-- ZEBRA MOTORS, DATABASE SCHEMA (Postgres variant)
-- ---------------------------------------------------------------------------
-- This is the production schema, applied automatically on first connection
-- once DATABASE_URL points at a real Postgres database (see
-- lib/db/client.js and lib/db/adapter.js's PostgresAdapter). It is generated
-- from, and kept in lockstep with, prisma/schema.sql (the SQLite schema used
-- in local dev): identical tables, columns, keys, and indexes. The only
-- difference is the timestamp DEFAULT expressions, which use Postgres's
-- now() instead of SQLite's datetime('now'). Every INSERT in lib/db/*.js
-- passes its own explicit ISO timestamp (nowIso()) anyway, so these DEFAULTs
-- are only ever a fallback, not something application code relies on.
--
-- If you add or change a table, make the same change in prisma/schema.sql
-- first, then mirror it here, there is no automatic sync between the two
-- files.
-- ---------------------------------------------------------------------------
