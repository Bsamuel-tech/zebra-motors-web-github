...
// Real photo upload, talks to /api/vehicles/[id]/photos. Shown only on the
// vehicle edit page (a vehicle needs to exist first). Until Zebra's real
// photo shoot happens, this list is simply empty and the public site shows
// the grey placeholder, nothing here fabricates or stands in a stock photo.
export default function VehiclePhotoManager({ vehicleId, photos }) {
  ...
  async function setPrimary(photo) {
    setPending(photo.id);
    await fetch(`/api/vehicles/${vehicleId}/photos/${photo.id}`, {
      method: "PATCH",
  ...
  async function remove(photo) {
    ...
    setPending(photo.id);
    await fetch(`/api/vehicles/${vehicleId}/photos/${photo.id}`, { method: "DELETE" });
    setPending(null);
    router.refresh();
  }
    ...
    <div style={{ maxWidth: 640, marginTop: 8 }}>
      ...
      {photos.length === 0 && (
        <p className="muted" style={{ fontSize: 13, marginBottom: 16 }}>
          ...
          vehicle until one is added here.
        </p>
      )}
